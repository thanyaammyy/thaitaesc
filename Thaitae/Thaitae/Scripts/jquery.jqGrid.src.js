...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		isSafari = ($.browser.webkit || $.browser.safari) && parseFloat($.browser.version)<536.5 ? true : false; // Chrome < version 19 (see http://de.wikipedia.org/wiki/Google_Chrome)
		...
		$("th",thr).each(function ( j ) {
			...
			var t = $(e.target).closest("th").find(s);
...
$.jgrid.extend({
	...
	showHideCol : function(colname,show) {
		return this.each(function() {
			var $t = this, fndh=false, brd=($.browser.webkit||$.browser.safari)&&(parseFloat($.browser.version)<536.5)? 0: $t.p.cellLayout, cw;
			if (!$t.grid ) {return;}
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			if (!this.grid ) {return;}
			var $t = this, cw,
			initwidth = 0, brd=($.browser.webkit||$.browser.safari)&&(parseFloat($.browser.version)<536.5)? 0: $t.p.cellLayout, lvc, vc=0, hs=false, scw=$t.p.scrollOffset, aw, gw=0,
			cl = 0,cr;
...
// Date Validation Javascript
	...
	checkValues : function(val, valref,g, customobject, nam) {
		var edtrul,i, nm, dft, len, isEmpty = $.jgrid.isEmpty;
		...
		if(edtrul) {
			...
			if(edtrul.required === true) {
				if( isEmpty(val) )  { return [false,nm+": "+$.jgrid.edit.msg.required,""]; }
			}
			// force required
