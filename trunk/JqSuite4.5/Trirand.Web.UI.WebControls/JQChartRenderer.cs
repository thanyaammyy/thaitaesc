...
namespace Trirand.Web.UI.WebControls
{
	internal class JQChartRenderer
	{
		...
		public string RenderHtml()
		{
			...
			{
				return "This is a trial version of jqSuite for ASP.NET WebForms which has expired.<br> Please, contact sales@trirand.net for purchasing the product or for trial extension.";
			}
		...
		private void RenderChartSettings(StringBuilder s)
		{
			...
			s.AppendFormat(",tooltip:{0}", this._chart.ToolTips.ToJSON());
			...
			hashtable.Add("url", HttpContext.Current.Request.Url.PathAndQuery);
			s.AppendFormat(",exporting:{0}", new JavaScriptSerializer().Serialize(hashtable));
